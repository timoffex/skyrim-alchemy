...
import           AlchemyInteraction
    ( AlchemyInteraction (..), IngredientForEffects (..) )
...
newtype AlchemyInteractionIO m a
  = AlchemyInteractionIO { unwrap :: m a }
...
instance ( Has (Lift IO) sig m
         ) => Algebra (AlchemyInteraction :+: sig)
                      (AlchemyInteractionIO m) where
