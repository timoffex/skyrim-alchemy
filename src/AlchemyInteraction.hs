...
data AlchemyInteraction (m :: Type -> Type) a where
  ...
  PrintError
    :: String -> AlchemyInteraction m ()

  PrintEffects
    :: S.Set EffectName -> AlchemyInteraction m ()
  PrintIngredients
    :: S.Set IngredientName -> AlchemyInteraction m ()
  PrintOrderedIngredientsForEffects
...
geq (PrintOrderedIngredientsForEffects _)
